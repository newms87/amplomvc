...
<div id='search_content'>
	...
	if(count($results)){
		$cols = 2;
		$count = 0;
		foreach($results as $product){ extract($product);?>
...
function open_product_view(href){
	...
	sc.data('href',href);
	sc.height(440);
	function cbox_closed_call(){sc.height(sc.data('orig_height'));}
...
function open_current_product(){open_product_view($('#search_content .product_list').data('href'));}
