...
<script type='text/javascript'>// <!--
	function toggleShowSearch(event, args) {
		...
		if (show) {
			...
			if (show === result.data('showing')) {
				...
				result.css({width: 0}).show().animate({width: result.data('width')}, {duration: 500, complete: function () {
					result.data('showing', true);
				}});
			...
			$.post(args.url, args.form.serialize(), function (data) {
				if (page > 1) {
					...
					if (page < parseInt(result.find('.load_more_products').attr('pages'))) {
						...
						result.find('.box_content').scroll(check_scroll_bottom);
					}
			...
			result.animate({width: 0}, {duration: 500, complete: function () {
				result.data('showing', false);
			}}).hide();
			$('#result_close').remove();
			result.html('');
	...
	function check_scroll_bottom(event) {
		trigger_offset = 20;
		scrollmax = $('#featured_filter_results .box-product').height() - $('#featured_filter_results .box_content').height();
	...
	$(document).ready(function () {
		...
		$('#menu_categories li').hover(function (event) {
			$(this).children('ul').each(function (i, e) {
				...
				if (animate) {
					height = $(e).height();
					$(e).height(0);
					$(e).animate({height: height}, {duration: 100, complete: function () {
						$(this).css('height', 'auto');
					}});
				}
			});
		}, function (event) {
			if ($(this).find('.active').length == 0)$(this).children('ul').hide();
		});
			...
			if ($(this).attr('filter') == 'category_id') {
				...
				$(this).siblings('ul').each(function (i, e) {
					...
					if (animate) {
						height = $(e).height();
						$(e).height(0);
						$(e).animate({height: height}, {duration: 300, complete: function () {
							$(this).css('height', 'auto');
						}});
					}
				});
		...
		$('#featured_filter_results .box_content').live('mousewheel', function (e, d) {
