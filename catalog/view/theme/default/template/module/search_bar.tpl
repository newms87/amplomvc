<div class="box-search">
	<form method='post' action='<?= $results_url; ?>' id='search_form' target='search_results' >
		...
		<ul>
			<li>
				<?= $this->builder->build_custom_select_dropdown($categories, 'search_category', array('display_name'=>'CATEGORY', 'value'=>""), $search_category); ?>
			</li>
			<li>
				<?
				$color_list = array();
				foreach($colors as $key=>$c){
					...
					$color_list[] = array('display_name'=>$c, 'value'=>$key, 'before'=>"<div class='color_circle' $color_style></div>");
				}?>
				<?= $this->builder->build_custom_select_dropdown($color_list, 'search_color', array('display_name'=>'COLOR', 'value'=>""), $search_color); ?>
			</li>
				<li>
				<?= $this->builder->build_custom_select_dropdown($styles, 'search_style', array('display_name'=>'STYLE', 'value'=>""), $search_style); ?>
			</li>
			<li>
				<?= $this->builder->build_custom_select_dropdown($countries, 'search_country', array('display_name'=>'COUNTRY', 'value'=>""), $search_country); ?>
			</li>
		</ul>
...
<script type='text/javascript'>
	// <!--
		function toggleShowSearch(event,args){
			...
			sr.animate({width:0},{duration:500, complete: function(){sr.data('showing',false);}}).hide();
			$('#search_close').remove();
			$('#search_results').html('');
		}
		...
		$(document).ready(function(){
			...
			$('#gen_search').focus(function(){if($(this).val() == 'SEARCH HERE')$(this).addClass('active').val('');});
			$('#gen_search').blur(function(){if($(this).val() == ''){$(this).removeClass('active').val('SEARCH HERE');}});
