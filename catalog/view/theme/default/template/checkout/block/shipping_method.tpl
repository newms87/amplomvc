...
<? } elseif(!empty($cart_error_shipping_method)) { ?>
	...
	<? if(!empty($allowed_shipping_zones)) { ?>
		...
		<? foreach($allowed_shipping_zones as $i => $geo_zone){ ?>
			<span class='allowed_zone_item'><?= $geo_zone['country']['name'] . (($i==count($allowed_shipping_zones)-1) ? '' : $text_zone_separator);?></span>
		<? } ?>
		</div>
	<? } ?>
...
<form action="<?= $validate_shipping_method; ?>" method="post">
	...
	<? foreach ($shipping_methods as $id => $shipping_method) { ?>
		...
		<tr class="shipping_method checkout_method highlight">
			<td class="method_id">
				<input type="radio" name="shipping_method" value="<?= $id; ?>" id="<?= $id; ?>" <?= $id == $shipping_method_id ? 'checked="checked"' : '';?> />
			</td>
