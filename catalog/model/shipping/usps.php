...
class Catalog_Model_Shipping_Usps extends Model 
{
	public function getQuote($address)
	{
		...
		if ($status) {
			...
			$weight = $this->weight->convert($this->cart->getWeight(), $this->config->get('config_weight_class _id'), $this->config->get('usps_weight_class_id'));
			...
			if ($status) {
				...
				if ($result) {
					if ($this->config->get('usps_debug')) {
						$this->error_log->write("USPS DATA SENT: " . urldecode($request));
						$this->error_log->write("USPS DATA RECV: " . $result);
					}
							...
							foreach ($services as $service) {
								...
								if (in_array($id, $allowed) && $this->config->get('usps_international_' . $id)) {
									...
									if ($this->config->get('usps_display_time')) {
										$title .= ' (' . $this->_('text_eta') . ' ' . $service->getElementsByTagName('SvcCommitments')->item(0)->nodeValue . ')';
									}
									...
									$quote_data[$id] = array(
										...
										'title'		=> $title,
										'cost'			=> $this->currency->convert($cost, 'USD', $this->config->get('config_currency')),
										'tax_class _id' => $this->config->get('usps_tax_class_id'),
										'text'			=> $this->currency->format($this->tax->calculate($this->currency->convert($cost, 'USD', $this->currency->getCode()), $this->config->get('usps_tax_class_id')), $this->currency->getCode(), 1.0000000)
									);
								}
							}
					...
					} elseif ($error) 
{
			...
			if ($quote_data) {
				...
				if ($this->config->get('usps_display_weight')) {
					$title .= ' (' . $this->_('text_weight') . ' ' . $this->weight->format($weight, $this->config->get('usps_weight_class_id')) . ')';
				}
