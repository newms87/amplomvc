...
class ControllerModuleFeatured extends Controller {
	protected function index($setting) {
		...
		if(isset($setting['fm_id'])){
			...
			if($setting['display'] == 'context'){
				...
				$setting['limit'] = 4;
				$this->data['spotlight'] = $this->getChild('module/featured/filter',array('setting'=>$setting, 'filter'=>'featured','category'=>-1));
				
				$setting['limit'] = 8;
				$this->data['ending_soon'] = $this->getChild("module/featured/filter",array('setting'=>$setting,'filter'=>'ending_soon','category'=>0));
	...
	public function filter($args=array()){
		...
		if(!$setting)
			$setting = array('limit'=>8,'image_width'=>174, 'image_height'=>135);
