...
class Catalog_Controller_Mail_Contact extends Controller
{
	public function index($contact_info)
	{
		...
		if (!isset($contact_info['name']) || !isset($contact_info['email'])) {
			trigger_error($this->_('error_contact_info') . get_caller(0,2));
		...
		$this->mail->setSubject(html_entity_decode(sprintf($this->_('email_subject'), $contact_info['name']), ENT_QUOTES, 'UTF-8'));
