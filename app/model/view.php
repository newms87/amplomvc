...
class App_Model_View extends App_Model_Table
{
	...
	public function save($view_id, $view)
	{
		...
		if ($view_id) {
			...
			if (!isset($view['sort_order'])) {
				$view['sort_order'] = (int)$this->queryVar("SELECT MAX(sort_order) FROM " . self::$prefix . "view WHERE `group` = '" . $this->escape($view['group']) . "'") + 1;
			}
	...
	{
		$view = $this->queryRow("SELECT * FROM " . self::$prefix . "view WHERE view_id = " . (int)$view_id);
	...
	{
		$settings = $this->queryVar("SELECT settings FROM " . self::$prefix . "view WHERE view_id = " . (int)$view_id);
	...
	{
		$views = $this->queryRows("SELECT * FROM " . self::$prefix . "view WHERE `group` = '" . $this->escape($group) . "'");
	...
	public function getViewMeta($view_id, $key = null, $single = true)
	{
		if (!isset(self::$meta[$view_id])) {
			$rows = $this->queryRows("SELECT * FROM " . self::$prefix . "view_meta WHERE view_id = " . (int)$view_id);
	...
	public function saveViewListing($view_listing_id, $view_listing)
	{
		if (!$view_listing_id || isset($view_listing['name'])) {
			...
			if (!$view_listing_id) {
				if ($this->queryVar("SELECT COUNT(*) FROM " . self::$prefix . "view_listing WHERE `name` = '" . $this->escape($view_listing['name']) . "'")) {
		...
		if (!empty($view_listing['sql'])) {
			...
			$this->query("DROP VIEW IF EXISTS `" . self::$prefix . "$slug`");

			$display_errors = ini_get('display_errors');
			ini_set('display_errors', 0);

			$result = $this->query("CREATE VIEW `" . self::$prefix . "$slug` AS " . $view_listing['sql']);
	...
	public function syncViewListing($view_listing)
	{
		...
		$view_listing_id = $this->queryVar("SELECT view_listing_id FROM " . self::$prefix . "view_listing WHERE `name` = '" . $this->escape($view_listing['name']) . "' AND `path` = '" . $this->escape($view_listing['path']) . "'");
	...
	{
		return $this->queryVar("SELECT slug FROM " . self::$prefix . "view_listing WHERE view_listing_id = " . (int)$view_listing_id);
	}
	...
	public function getAllViewListings()
	{
		if (!self::$view_listings) {
			...
			if (!self::$view_listings) {
				self::$view_listings = $this->queryRows("SELECT * FROM " . self::$prefix . "view_listing ORDER BY name", 'view_listing_id');

				if (!self::$view_listings) {
					//Initialize the View Listings if the table is empty
					if (!$this->queryVar("SELECT COUNT(*) FROM " . self::$prefix . "view_listing")) {
