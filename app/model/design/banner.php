<?php
class App_Model_Design_Banner extends Model
{
	...
	{
		return $this->queryRow("SELECT * FROM " . DB_PREFIX . "banner WHERE banner_id = " . (int)$banner_id);
	}

	public function getActiveBanner($banner_id)
	{
		return $this->queryRows("SELECT * FROM " . DB_PREFIX . "banner_image bi LEFT JOIN " . DB_PREFIX . "banner_image_description bid ON (bi.banner_image_id  = bid.banner_image_id) WHERE bi.banner_id = '" . (int)$banner_id . "' AND bid.language_id = '" . (int)option('config_language_id') . "' ORDER BY bi.sort_order");
	}
