...
<div class="section">
	...
	<div class="box">
		<div class="heading">
			<h1>
				<img src="<?= theme_url('image/banner.png'); ?>" alt=""/> <?= _l("Banners"); ?></h1>

			<div class="buttons">
				<a onclick="$('#form').submit();" class="button"><?= _l("Save"); ?></a>
				<a
					href="<?= $cancel; ?>" class="button"><?= _l("Cancel"); ?></a>
			</div>
		</div>
		<div class="section">
				...
				<table id="images" class="list">
					...
					<? foreach ($banner_images as $banner_image) { ?>
						<tr class="imagerow" id="image-row<?= $image_row; ?>">
							...
							<td class="right">
								<input class="sortOrder" type="text" name="banner_image[<?= $image_row; ?>][sort_order]" value="<?= $banner_image['sort_order']; ?>" size="2"/>
							</td>
							<td class="left">
								<a onclick="$('#image-row<?= $image_row; ?>').remove();"
									class="button"><?= _l("Remove"); ?></a>
							</td>
						</tr>
						<? $image_row++; ?>
					<? } ?>
					...
					<tr>
						<td colspan="3"></td>
						<td class="left">
							<a onclick="image_manager();" class="button">File Manager</a>
						</td>
					</tr>
					</tfoot>
				</table>
