<?php
class Admin_Controller_Plugin_Plugin extends Controller
{
		...
		foreach ($plugins as &$plugin) {
			if ($plugin['installed']) {
				$plugin['actions'] = array( /*'edit' => array(
						'text' => _l("Edit"),
						'href' => $this->url->link('plugin/plugin/update', 'name=' . $plugin['name']),
					),*/
				);
				...
				if ($this->plugin->hasChanges($plugin['name'])) {
					$plugin['actions']['add_changes'] = array(
						'text' => _l("Add Changes"),
						'href' => $this->url->link('plugin/plugin/add_changes', 'name=' . $plugin['name']),
					);
				}
				...
				if ($this->Model_Setting_Plugin->canUninstall($plugin['name'])) {
					$plugin['actions']['uninstall'] = array(
						'text' => _l("Uninstall"),
						'href' => $this->url->link('plugin/plugin/uninstall', 'name=' . $plugin['name']),
					);
				} else {
				...
				if ($this->Model_Setting_Plugin->canInstall($plugin['name'])) {
					$plugin['actions'] = array(
						'install' => array(
							'text' => _l("Install"),
							'href' => $this->url->link('plugin/plugin/install', 'name=' . $plugin['name']),
						)
					);
				} else {
					$plugin['actions'] = array(
	...
	{
		if (!$this->user->can('modify', 'plugin/plugin')) {
