    ...
    <div class="content">
      <form action="<?= $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div class="vtabs">
          ...
          <a href="#tab-module-<?= $module_row; ?>" id="module-<?= $module_row; ?>"><div id='tab-module-<?= $module_row?>-title' class="tab_title"><?= $module['title'][$lang_id]; ?></div>&nbsp;<img src="view/image/delete.png" alt="" onclick="$('.vtabs a:first').trigger('click'); $('#module-<?= $module_row; ?>').remove(); $('#tab-module-<?= $module_row; ?>').remove(); return false;" /></a>
          <? $module_row++; ?>
          <? } ?>
        ...
        <div id="tab-module-<?= $module_row; ?>" class="vtabs-content">
          <div id="language-<?= $module_row; ?>" class="htabs">
            ...
            <a href="#tab-language-<?= $module_row; ?>-<?= $language['language_id']; ?>"><img src="view/image/flags/<?= $language['image']; ?>" title="<?= $language['name']; ?>" /> <?= $language['name']; ?></a>
            <? }} ?>
          </div>
                	...
                	<? if(isset($module['data']))foreach($module['data'] as $md){ ?>
                	  <div class='slide_data' slidenum="<?=$md_row;?>">
	                	  ...
	                	  <input type="hidden" name="dn_carousel_module[<?= $module_row; ?>][data][<?= $md_row;?>][article_id]" value="<?= $md['article_id']; ?>" />
	            			...
	            			<br />
	               			<a onclick="el_uploadSingle('image<?= "$module_row-$md_row"; ?>', 'thumb<?= "$module_row-$md_row"; ?>');"><?= $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;
	                	  ...
	                	  <textarea name="dn_carousel_module[<?= $module_row; ?>][data][<?= $md_row;?>][description]"><?= isset($md['description']) ? $md['description']:''; ?></textarea>
                	  	  <img class='remove_slide_img' src="view/image/delete.png" alt="" onclick="remove_article_slide(<?= $module_row . ',' . $md_row;?>);" />
                	  </div>
                	  <? $md_row++;?>
                	<? } ?>
				</td>
...
function addModule() {	
    ...
    <? foreach ($languages as $language) {if($language['status']){ ?>
    html += '    <a href="#tab-language-'+ module_row + '-<?= $language['language_id']; ?>"><img src="view/image/flags/<?= $language['image']; ?>" title="<?= $language['name']; ?>" /> <?= $language['name']; ?></a>';
    <? }} ?>
	html += '  </div>';
	...
	$('#module-add').before('<a href="#tab-module-' + module_row + '" id="module-' + module_row + '"><div id="tab-module-'+module_row+'-title" class="tab_title"><?= $tab_carousel; ?> ' + module_row + '</div>&nbsp;<img src="view/image/delete.png" alt="" onclick="$(\'.vtabs a:first\').trigger(\'click\'); $(\'#module-' + module_row + '\').remove(); $(\'#tab-module-' + module_row + '\').remove(); return false;" /></a>');
...
function add_article_slide(modrow, title, article_id){
	...
	html += '<br />'
	html += "<a onclick=\"el_uploadSingle('image" + modrow + '-' + md_row + "', 'thumb" + modrow + '-' + md_row + "');\"><?= $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp";
	...
	html += '  <img class="remove_slide_img" src="view/image/delete.png" alt="" onclick="remove_article_slide(' + modrow + ',' + md_row + ');" />';
	html += '</div>';
