...
<? } ?>
			...
			<form action="<?= $action; ?>" method="post" enctype="multipart/form-data" id="form">
				<table id="module" class="list">
					...
					<? foreach ($modules as $module) { ?>
						...
						<tr>
							<td class="left"><select name="main_sidebar_module[<?= $module_row; ?>][layout_id]">
	...
	<script type="text/javascript"><!--
		...
		function addModule() {
			...
			html += '	<tr>';
			html += '		<td class="left"><select name="main_sidebar_module[' + module_row + '][layout_id]">';
			<? foreach ($layouts as $layout) { ?>
			html += '			<option value="<?= $layout['layout_id']; ?>"><?= addslashes($layout['name']); ?></option>';
			<? } ?>
			html += '		</select></td>';
			html += '		<td class="left"><select name="main_sidebar_module[' + module_row + '][position]">';
			...
			html += '		</select></td>';
			html += '		<td class="left"><select name="main_sidebar_module[' + module_row + '][status]">';
			html += '			<option value="1" selected="selected"><?= $text_enabled; ?></option>';
			html += '			<option value="0"><?= $text_disabled; ?></option>';
			html += '		</select></td>';
			html += '		<td class="right"><input type="text" name="main_sidebar_module[' + module_row + '][sort_order]" value="" size="3" /></td>';
