<?= $header; ?>
<div class="section">
	<?= $this->breadcrumb->render(); ?>
	<div class="box">
		<div class="heading">
			<h1><img src="<?= HTTP_THEME_IMAGE . 'log.png'; ?>" alt=""/> <?= _l("Error Log"); ?></h1>

			<div class="buttons">
				<? if ($prev || $next) { ?>
					<?= _l("Limit"); ?> <input type="text" id="limit" value="<?= $limit; ?>" onchange="update_limit();"/>
				<? } ?>
				<? if ($prev) { ?>
					<a id="button_prev" href="<?= $prev; ?>" class="button"><?= _l("Previous"); ?></a>
				<? } ?>
				<? if ($next) { ?>
					<a id="button_next" href="<?= $next; ?>" class="button"><?= _l("Next"); ?></a>
				<? } ?>
			</div>
		</div>
		<div class="section">
			<table class="form">
				<tr>
					<td>
						<div><?= _l("Remove"); ?></div>
						<span class="help">(eg: 1,2-4,34,58,60-89)</span>
					</td>
					<td>
						<form action="<?= $remove; ?>" method="post" style="float:left;">
							<input id="remove_entries" type="text" name="entries" value=""/>
							<a class="button" onclick="$(this).closest('form').submit();"><?= _l("Remove"); ?></a>
						</form>
						<a href="<?= $clear; ?>" class="button"
							style="float:left;margin-left:20px"><?= _l("Clear Log"); ?></a>
					</td>
				</tr>
				<tr>
					<td><?= _l("Filter Log Entries:"); ?></td>
					<td id="filter_types">
						<? $this->builder->setConfig('store_id', 'name', 'string'); ?>
						<?= $this->builder->build('select', $stores, 'filter_store', $filter_store); ?>
						<a id="filter_link" class="button" href="<?= $filter_url; ?>" onclick="return set_filter_url();"><?= _l("Apply Filter"); ?></a>
					</td>
				</tr>
			</table>
			<table class="list" width="100%">
				<thead>
					<tr>
						<td width="2%"><?= _l("Remove"); ?></td>
						<td width="2%"><?= _l("Line:"); ?></td>
						<td width="6%"><?= _l("Date:"); ?></td>
						<td width="3%"><?= _l("Remote IP:"); ?></td>
						<td width="65%"><?= _l("Message:"); ?></td>
						<td width="8%"><?= _l("URL Accessed:"); ?></td>
						<td width="8%"><?= _l("Query Requested:"); ?></td>
						<td width="8%"><?= _l("Store:"); ?></td>
						<td width="8%"><?= _l("User Agent:"); ?></td>
					</tr>
				</thead>
				<tbody>
					<? foreach ($log as $l) { ?>
						<tr id="entry<?= $l['line']; ?>">
							<td><a class="button" onclick="remove_entry(<?= (int)$l['line']; ?>);"><?= 'X'; ?></a></td>
							<td><?= $l['line']; ?></td>
							<td><?= $l['date']; ?></td>
							<td><?= $l['ip']; ?></td>
							<td width="65%"><?= $l['message']; ?></td>
							<td><?= $l['uri']; ?></td>
							<td><?= $l['query']; ?></td>
							<td><?= $l['store']; ?></td>
							<td><?= $l['agent']; ?></td>
						</tr>
					<? } ?>
				</tbody>
			</table>
		</div>
	</div>
</div>
<script type="text/javascript">
	function remove_entry(line) {
		$.post("<?= $remove; ?>", {entries: line, no_page: 1},
			function (msg) {
				$('#entry' + line).css({height: 50})
					.html("<td colspan="
				7
				" style="
				position:relative;
				padding:5
				px
				"><span style="
				position:absolute;
				top:5
				px;
				" class="
				message_box
				success
				">" + msg + "</span></td>"
				)
				;
				setTimeout(function () {
					$('#entry' + line).fadeOut(300);
				}, 1000);
			}
		);
		$('#entry' + line).html("<td colspan="
		7
		" style="
		height:50
		px;
		"><img src="
		<?= $loading; ?>" /></td>"
	)
		;
	}

	$('#button_prev, #button_next').trigger('change');
	function update_limit() {
		limit = $('#limit').val();
		$('#button_prev, #button_next').each(function (i, e) {
			$(e).attr('href', $(e).attr('href').replace(/&limit=\d*/gi, '') + '&limit=" + limit);
		});
	}

	function set_filter_url() {
		filters = "';

		$('#filter_types select').each(function (i, e) {
			filters += $(e).val() ? '&' + $(e).attr('name') + '=" + $(e).val() : "';
		});

		$('#filter_link').attr('href', '<?= $filter_url; ?>' + filters);
		return true;
	}

</script>
<?= $footer; ?>
