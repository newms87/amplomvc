					...
					<table class="form">
						<? foreach ($languages as $language) { ?>
							<tr>
								...
								<td><textarea name="bank_transfer_bank_<?= $language['language_id']; ?>" cols="80"
								              rows="10"><?= isset(${'bank_transfer_bank_' . $language['language_id']}) ? ${'bank_transfer_bank_' . $language['language_id']} : ''; ?></textarea>
									<img src="<?= HTTP_THEME_IMAGE . 'flags/<?= $language['image']; ?>'; ?>"
									     title="<?= $language['name']; ?>" style="vertical-align: top;"/><br/>
						...
						<tr>
							...
							<td><input type="text" name="bank_transfer_sort_order" value="<?= $bank_transfer_sort_order; ?>"
							           size="1"/></td>
						</tr>
					</table>
				</form>
