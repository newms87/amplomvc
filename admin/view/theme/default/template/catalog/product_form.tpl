...
<div id="tab-data">
	<table class="form">
		...
		<tr>
			...
			<td>
				<input type="text" onfocus="$(this).display_error('<?= $warning_generate_url; ?>', 'gen_url');" name="alias"
				       value="<?= $alias; ?>"/>
		...
		<tr>
			...
			<td><input type="text" name="date_available" value="<?= $date_available; ?>" size="12" class="datetimepicker"/>
			</td>
		</tr>
		<tr>
			<td><?= $entry_date_expires; ?></td>
			<td><input type="text" name="date_expires" value="<?= $date_expires; ?>" size="12" class="datetimepicker"/>
			</td>
		</tr>
		<tr>
			<td><?= $entry_status; ?></td>
...
<div id="tab-option">
	<div id="vtab-option" class="vtabs">
		...
		<div id="option-add">
			...
			<img src="<?= HTTP_THEME_IMAGE . 'add.png'; ?>" alt="<?= $button_add_option; ?>"
			     title="<?= $button_add_option; ?>"/>
		</div>
	...
	<div id="product_option_list">
		<? foreach ($product_options as $row => $product_option) { ?>
			<div id="tab-option-<?= $row; ?>" class="product_option vtabs-content" data-row="<?= $row; ?>"
			     data-id="<?= $product_option['option_id']; ?>">
				<input type="hidden" name="product_options[<?= $row; ?>][product_option_id]"
				       value="<?= $product_option['product_option_id']; ?>"/>
				<input type="hidden" name="product_options[<?= $row; ?>][name]" value="<?= $product_option['name']; ?>"/>
				<input type="hidden" name="product_options[<?= $row; ?>][display_name]"
				       value="<?= $product_option['display_name']; ?>"/>
				<input type="hidden" name="product_options[<?= $row; ?>][option_id]"
				...
				<table class="form">
					...
					<tr>
						<td><?= $entry_sort_order; ?></td>
						<td><input type="text" name="product_options[<?= $row; ?>][sort_order]"
						           value="<?= $product_option['sort_order']; ?>"/></td>
					</tr>
					<tr>
						...
						<td>
							<div class='scrollbox unused_option_value_list clickable'>
								<? foreach ($product_option['unused_option_values'] as $uov_row => $option_value) { ?>
									<div class="unused_option_value" data-row="<?= $uov_row; ?>"
									     data-id="<?= $option_value['option_value_id']; ?>" onclick="add_option_value($(this));">
					...
					<? if (!empty($product_option['product_option_values'])) { ?>
						<? foreach ($product_option['product_option_values'] as $pov_row => $product_option_value) { ?>
							...
							<tr class="product_option_value" data-row="<?= $pov_row; ?>">
								<td class="center">
									<input type="hidden" name="<?= $product_option_value_row; ?>[product_option_value_id]"
									       value="<?= $product_option_value['product_option_value_id']; ?>"/>
									<input type="hidden" name="<?= $product_option_value_row; ?>[option_value_id]"
									       value="<?= $product_option_value['option_value_id']; ?>"/>
									<input type="hidden" name="<?= $product_option_value_row; ?>[value]"
									       value="<?= $product_option_value['value']; ?>"/>
									<span class='option_value_label'><?= $product_option_value['value']; ?></span>
								</td>
								<td class="center">
									<input type="radio" name="<?= $product_option_value_row; ?>[default]"
									       value="1" <?= $product_option_value['default'] ? 'checked="checked"' : ''; ?> />
								</td>
								<td class="center">
								...
								<td class="center"><input type="text" name="<?= $product_option_value_row; ?>[quantity]"
								                          value="<?= $product_option_value['quantity']; ?>" size="3"/></td>
								<td class="center">
									<?= $this->builder->build('select', $data_yes_no, $product_option_value_row . "[subtract]", $product_option_value['subtract']); ?>
								</td>
								<td class="center">
									<input type="text" name="<?= $product_option_value_row; ?>[cost]"
									       value="<?= $product_option_value['cost']; ?>" size="5"/>
								</td>
								<td class="center">
									<input type="text" name="<?= $product_option_value_row; ?>[price]"
									       value="<?= $product_option_value['price']; ?>" size="5"/>
								</td>
								<td class="center">
									<input type="text" name="<?= $product_option_value_row; ?>[points]"
									       value="<?= $product_option_value['points']; ?>" size="5"/>
								</td>
								<td class="center">
									<input type="text" name="<?= $product_option_value_row; ?>[weight]"
									       value="<?= $product_option_value['weight']; ?>" size="5"/>
								</td>
								<td class="center">
									<input type="text" class="sort_order" name="<?= $product_option_value_row; ?>[sort_order]"
									       value="<?= $product_option_value['sort_order']; ?>" size="5"/>
								</td>
								<td class="center">
									<table class='list'>
										<thead>
										...
										<? if (!empty($product_option_value['restrictions'])) { ?>
											<? foreach ($product_option_value['restrictions'] as $r_row => $restriction) { ?>
												...
												<tr class="product_option_value_restriction" data-row="<?= $r_row; ?>">
													...
													<td class="center"><input type="text" size='3'
													                          name="<?= $restriction_row; ?>[quantity]"
													                          value="<?= $restriction['quantity']; ?>"/></td>
	...
	<table id="discount" class="list">
		...
		<? foreach ($product_discounts as $row => $product_discount) { ?>
			<tr class="product_discount" data-row="<?= $row; ?>">
				...
				<td class="right"><input type="text" name="product_discounts[<?= $row; ?>][quantity]"
				                         value="<?= $product_discount['quantity']; ?>" size="2"/></td>
				<td class="right"><input type="text" name="product_discounts[<?= $row; ?>][priority]"
				                         value="<?= $product_discount['priority']; ?>" size="2"/></td>
				<td class="right"><input type="text" name="product_discounts[<?= $row; ?>][price]"
				                         value="<?= $product_discount['price']; ?>"/></td>
				<td class="left"><input type="text" name="product_discounts[<?= $row; ?>][date_start]"
				                        value="<?= $product_discount['date_start']; ?>" class="datetimepicker"/></td>
				<td class="left"><input type="text" name="product_discounts[<?= $row; ?>][date_end]"
				                        value="<?= $product_discount['date_end']; ?>" class="datetimepicker"/></td>
	...
	<table id="special" class="list">
		...
		<? foreach ($product_specials as $row => $product_special) { ?>
			<tr class="product_special" data-row="<?= $row; ?>">
				...
				<td class="right"><input type="text" name="product_specials[<?= $row; ?>][priority]"
				                         value="<?= $product_special['priority']; ?>" size="2"/></td>
				<td class="right"><input type="text" name="product_specials[<?= $row; ?>][price]"
				                         value="<?= $product_special['price']; ?>"/></td>
				<td class="left"><input type="text" name="product_specials[<?= $row; ?>][date_start]"
				                        value="<?= $product_special['date_start']; ?>" class="datetimepicker"/></td>
				<td class="left"><input type="text" name="product_specials[<?= $row; ?>][date_end]"
				                        value="<?= $product_special['date_end']; ?>" class="datetimepicker"/></td>
				<td class="left"><a onclick="$(this).closest('.product_special').remove();"
				                    class="button"><?= $button_remove; ?></a></td>
			</tr>
		<? } ?>
		</tbody>
	...
	<table id="images" class="list">
		...
		<? foreach ($product_images as $row => $product_image) { ?>
			<tr class="product_image" data-row="<?= $row; ?>">
				...
				<td class="center"><input class="sort_order" type="text" name="product_images[<?= $row; ?>][sort_order]"
				                          value="<?= $product_image['sort_order']; ?>" size="2"/></td>
