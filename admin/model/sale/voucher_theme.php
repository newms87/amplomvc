...
class ModelSaleVoucherTheme extends Model {
	public function addVoucherTheme($data) {
		...
		foreach ($data['voucher_theme_description'] as $language_id => $value) {
			$this->query("INSERT INTO " . DB_PREFIX . "voucher_theme_description SET voucher_theme_id = '" . (int)$voucher_theme_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "'");
		}
	...
	public function getVoucherThemes($data = array()) {
      	if ($data) {
			...
			if (!$voucher_theme_data) {
				$query = $this->query("SELECT * FROM " . DB_PREFIX . "voucher_theme vt LEFT JOIN " . DB_PREFIX . "voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) WHERE vtd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY vtd.name");
