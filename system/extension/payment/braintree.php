...
class System_Extension_Payment_Braintree extends System_Extension_Payment
{
	...
	{
		if (!$this->initAPI()) {
			$url_contact = site_url('page/page', 'page_id=' . $this->config->get('config_contact_page_id'));
		...
		if ($new_card) {
			...
			if (!empty($new_card['save_to_account']) && $this->customer->isLogged()) {
				...
				if ($result->success) {
					...
					$url_contact = site_url('page/page', 'page_id=' . $this->config->load('config', 'config_contact_page_id'));
