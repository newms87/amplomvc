...
class Weight extends Library
{
	...
	public function __construct()
	{
		...
		$weight_classes = $this->queryRows("SELECT * FROM " . DB_PREFIX . "weight_class wc LEFT JOIN " . DB_PREFIX . "weight_class_description wcd ON (wc.weight_class_id = wcd.weight_class_id) WHERE wcd.language_id = '" . (int)option('config_language_id') . "'");

		foreach ($weight_classes as $weight_class) {
			$this->weights[$weight_class['weight_class_id']] = $weight_class;
		}

		$this->weight_class_id = option('config_weight_class_id');
	}
