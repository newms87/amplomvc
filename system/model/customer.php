...
class System_Model_Customer extends Model
{
	public function addCustomer($data)
	{
		...
		$customer_id = $data['customer_id'] = $this->insert('customer', $data);
		...
		if (!empty($data['metadata'])) {
			foreach ($data['metadata'] as $key => $value) {
				$this->setMetaData($customer_id, $key, $value);
			}
		}
	...
	public function setMetaData($customer_id, $key, $value)
	{
		$this->query("DELETE FROM " . DB_PREFIX . "customer_meta WHERE customer_id = '" . (int)$customer_id . "' AND `key` = '" . $this->escape($key) . "'");
		...
		$this->query("INSERT INTO " . DB_PREFIX . "customer_meta SET customer_id = '" . (int)$customer_id . "', `key` = '" . $this->escape($key) . "', `value` = '" . $this->db->escape($value) . "', serialized = '$serialized'");
	}

	public function deleteMetaData($customer_id, $key)
	{
		$this->query("DELETE FROM " . DB_PREFIX . "customer_meta WHERE customer_id = " . (int)$customer_id . " AND `key` = '" . $this->escape($key) . "'");
	}

	public function getMetaData($customer_id)
	{
		$metainfo = $this->queryRows("SELECT * FROM " . DB_PREFIX . "customer_meta WHERE customer_id = '" . (int)$customer_id . "'");

		$metadata = array();

		foreach ($metainfo as $meta) {
			if ($meta['serialized']) {
				$metadata[$meta['key']] = unserialize($meta['value']);
			} else {
