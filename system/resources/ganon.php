	...
	function next_pos($needle, $callback = true)
	{
		...
		if (($this->pos < $this->size) && (($p = stripos($this->doc, $needle, $this->pos + 1)) !== false)) {
			...
			if ($needle == '>') {
				...
				if (($php_pos = stripos($this->doc, '<?', $this->pos)) !== false && $php_pos < $p) {
					$p = stripos($this->doc, $needle, stripos($this->doc, '?>', $php_pos + 1));
				}
			}
...
class HTML_Node
{
	...
	function html()
	{
		dom_format($this, array('minify_script' => false));
	...
	function resolve($tag)
	{
		if (!is_object($tag)) {
			if (is_string($tag) && strpos(trim($tag), '<') === 0) {
				return str_get_dom($tag);
			} else {
				return new $this->childClass($tag, $this);
			}
		}
	...
	function parent()
	{
		return $this->parent ? $this->parent : str_get_dom("<div />");
	}
	...
	function closest($tag)
	{
		...
		while (is_object($parent = $parent->parent)) {
			if ($parent->is($tag)) {
				return $parent;
			}
		}

		return str_get_dom("<div />");
	}
	...
	function is($tag)
	{
		...
		if (strpos($tag, '.') === 0) {
			return $this->hasClass(substr($tag, 1));
		}
		if (strpos($tag, '#') === 0) {
			return $this->hasAttribute('id', substr($tag, 1));
		}
	}
...
class HTML_Selector
{
	...
	protected function error($error)
	{
		$error = htmlentities(str_replace(
			...
			array(
			     $this->parser->getTokenString(),
			     (int)$this->parser->getPos()
			),
			$error
		));
		trigger_error($error);
...
class HTML_Formatter
{
	...
	{
		$this->indent_string = $this->options['indent'];
