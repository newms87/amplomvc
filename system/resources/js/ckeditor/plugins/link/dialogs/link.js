...
CKEDITOR.dialog.add('link', function (a) {
    var b = CKEDITOR.plugins.link, c = function () {
        ...
        if (I = H.match(e))if (y == 'encode')H = H.replace(l, function (ac, ad, ae) {
            return 'mailto:' + String.fromCharCode.apply(String, ad.split(',')) + (ae && w(ae));
        }); else if (y)H.replace(m, function (ac, ad, ae) {
            if (ad == z.name) {
                M.type = 'email';
                var af = M.email = {}, ag = /[^,\s]+/g, ah = /(^')|('$)/g, ai = ae.match(ag), aj = ai.length, ak, al;
                for (var am = 0; am < aj; am++) {
                    al = decodeURIComponent(w(ai[am].replace(ah, '')));
                    ak = z.params[am].toLowerCase();
                    af[ak] = al;
                }
                af.address = [af.name, af.domain].join('@');
            }
        });
        ...
        if (G) {
            ...
            var V = this, W = function (ac, ad) {
                var ae = G.getAttribute(ad);
                if (ae !== null)M.adv[ac] = ae || '';
            };
            W('advId', 'id');
            W('advLangDir', 'dir');
        ...
        var X = new CKEDITOR.dom.nodeList(F.document.$.anchors), Y = M.anchors = [], Z;
        for (var aa = 0, ab = X.count(); aa < ab; aa++) {
            Z = X.getItem(aa);
            Y[aa] = {name: Z.getAttribute('name'), id: Z.getAttribute('id')};
        }
    ...
    return{title: E.title, minWidth: 350, minHeight: 230, contents: [
        ...
        switch (H.type || 'url') {
            case 'url':
                var K = H.url && H.url.protocol != undefined ? H.url.protocol : 'http://', L = H.url && H.url.url || '';
