...
// add lang file to session for easy include
	...
	<head>
		...
		<link rel="shortcut icon" href="<?= $fm_url . 'filemanager/img/ico/favicon.ico'; ?>">
		<link href="<?= $fm_url . 'filemanager/css/bootstrap.min.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/css/bootstrap-responsive.min.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/css/bootstrap-lightbox.min.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/css/style.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/css/dropzone.min.css'; ?>" type="text/css" rel="stylesheet"/>
		<?php
		$sprite_lang_file  = 'filemanager/img/spritemap_en_EN.png';
		$sprite_lang_file2 = 'filemanager/img/spritemap@2x_en_EN.png';
		?>
		<style>
			...
			.dropzone-previews .dz-preview .dz-progress .dz-upload {
				background-image: url(<?= $fm_url . $sprite_lang_file; ?>);
			}
			...
			@media all and (-webkit-min-device-pixel-ratio: 1.5),(min--moz-device-pixel-ratio: 1.5),(-o-min-device-pixel-ratio: 1.5/1),(min-device-pixel-ratio: 1.5),(min-resolution: 138dpi),(min-resolution: 1.5dppx) {
				...
				.dropzone .dz-preview .dz-progress .dz-upload,
				.dropzone-previews .dz-preview .dz-progress .dz-upload {
					background-image: url(<?= $fm_url . $sprite_lang_file2; ?>);
				}
			}
		</style>
		<link href="<?= $fm_url . 'filemanager/css/jquery.contextMenu.min.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/css/bootstrap-modal.min.css'; ?>" rel="stylesheet" type="text/css"/>
		<link href="<?= $fm_url . 'filemanager/jPlayer/skin/blue.monday/jplayer.blue.monday.css'; ?>" rel="stylesheet" type="text/css">
		<!--[if lt IE 8]>
		<style>
		...
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/bootstrap.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/bootstrap-lightbox.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/dropzone.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/jquery.touchSwipe.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/modernizr.custom.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/bootbox.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/bootstrap-modal.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/bootstrap-modalmanager.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/jPlayer/jquery.jplayer.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/imagesloaded.pkgd.min.js'; ?>"></script>
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/jquery.queryloader2.min.js'; ?>"></script>
		<?php
		if ($aviary_active) {
		...
		<![endif]-->
		<script src="<?= $fm_url . 'filemanager/js/jquery.ui.position.min.js'; ?>" type="text/javascript"></script>
		<script src="<?= $fm_url . 'filemanager/js/jquery-ui-1.10.4.custom.js'; ?>" type="text/javascript"></script>
		<script src="<?= $fm_url . 'filemanager/js/jquery.contextMenu.min.js'; ?>" type="text/javascript"></script>
		...
		<script>
			...
			Dropzone.options.myAwesomeDropzone = {
				...
				url:                 "<?= site_url('admin/filemanager/upload'); ?>",
			...
			if (image_editor) {
				var featherEditor = new Aviary.Feather({
					...
					onSave:   function (imageID, newURL) {
						...
						$.ajax({
							type: "POST",
							url:  "filemanager/ajax_calls?action=save_img",
							data: {
								url:  newURL,
		...
		<script type="text/javascript" src="<?= $fm_url . 'filemanager/js/include.min.js'; ?>"></script>
	</head>
	<body>
	...
	<div class="container-fluid">
							...
							<div class="row-fluid">
								...
								<div class="span3 half view-controller">
									...
									<button class="btn tip<?php if ($view == 0) {
										echo " btn-inverse";
									} ?>" id="view0" data-value="0" title="<?php echo lang_View_boxes; ?>">
										<i class="icon-th <?php if ($view == 0) {
											echo "icon-white";
										} ?>"></i></button>
									<button class="btn tip<?php if ($view == 1) {
										echo " btn-inverse";
									} ?>" id="view1" data-value="1" title="<?php echo lang_View_list; ?>">
										<i class="icon-align-justify <?php if ($view == 1) {
											echo "icon-white";
										} ?>"></i></button>
									<button class="btn tip<?php if ($view == 2) {
										echo " btn-inverse";
									} ?>" id="view2" data-value="2" title="<?php echo lang_View_columns_list; ?>">
		...
		<div class="row-fluid">
			<?php
			$link = site_url('admin/filemanager', $get_params);
			?>
			<ul class="breadcrumb">
				...
				<li class="pull-right">
					<a id="refresh" class="btn-small" href="<?= site_url('admin/filemanager', $get_params . $subdir . "&" . uniqid()); ?>"><i class="icon-refresh"></i></a>
				</li>
		...
		<div class="row-fluid ff-container">
			<div class="span12">
				<?php if (@opendir($current_path . $rfm_subfolder . $subdir) === false){ ?>
					<br/>
					<div class="alert alert-error">There is an error! The upload folder <?= $current_path . $rfm_subfolder . $subdir; ?> there isn't. Check your config.php file.</div>
				<?php }else{ ?>
				...
				<ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
					...
					foreach ($files as $file_array) {
						...
						<li data-name="<?php echo $file ?>" <?php if ($file == '..') {
							...
							<figure data-name="<?php echo $file ?>" class="<?php if ($file == "..") {
								...
								<a class="folder-link" href="<?= site_url('admin/filemanager', $get_params . rawurlencode($src) . "&" . uniqid()); ?>">
									<div class="img-precontainer">
										<div class="img-container directory"><span></span>
											<img class="directory-img" src="<?= $fm_url_img . $icon_theme . '/folder' . ($file === ".." ? "_back" : '') . '.jpg'; ?>" alt="folder"/>
										</div>
									</div>
									<div class="img-precontainer-mini directory">
										<div class="img-container-mini">
											<span></span>
											<img class="directory-img" src="<?= $fm_url_img . $icon_theme . '/folder' . ($file === ".." ? "_back" : '') . '.png'; ?>" alt="folder"/>
										</div>
									</div>
									<?php if ($file == ".."){ ?>
					...
					<li class="ff-item-type-<?php echo $class_ext; ?> file" data-name="<?php echo $file; ?>"><?php
						...
						<figure data-name="<?php echo $file ?>" data-type="<?php if ($is_img) {
							...
							<figcaption>
								<form action="<?= site_url('admin/filemanager/force_download'); ?>" method="post" class="download-form" id="form<?php echo $nu; ?>">
									...
									<?php } elseif (($is_video || $is_audio) && in_array($extension_lower, $jplayer_ext)) { ?>
										<a class="tip-right modalAV <?php if ($is_audio) {
											...
											title="<?php echo lang_Preview ?>" data-url="<?= site_url('admin/filemanager/ajax_calls', 'action=media_preview&title=' . $filename . '&file=' . $current_path . $rfm_subfolder . $subdir . $file); ?>" href="javascript:void('');"><i class=" icon-eye-open"></i></a>
									<?php } else { ?>
	...
	<div id="loading_container" style="display:none;">
		...
		<img id="loading_animation" src="<?= $fm_url_img . 'storing_animation.gif'; ?>" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
	</div>
	<!-- loading div end -->
